...
CREATE TABLE payments (
    ...
    CONSTRAINT fk_payment_consultation FOREIGN KEY (consultation_room_id) 
        REFERENCES consultation_room(room_id)
);

-- ConsultationRoom 테이블에 is_paid 컬럼 추가
ALTER TABLE consultation_room ADD is_paid NUMBER(1) DEFAULT 0;

-- 인덱스 생성
-- order_id는 UNIQUE 제약조건으로 이미 인덱스가 생성되므로 생략
-- CREATE INDEX idx_payment_order_id ON payments(order_id);
CREATE INDEX idx_payment_key ON payments(payment_key);
CREATE INDEX idx_payment_consultation_room ON payments(consultation_room_id);
...
COMMENT ON COLUMN payments.method IS '결제 방법';
COMMENT ON COLUMN payments.status IS '결제 상태 (PENDING, COMPLETED, CANCELLED, FAILED)';
COMMENT ON COLUMN payments.consultation_room_id IS '상담방 ID';
COMMENT ON COLUMN payments.approved_at IS '결제 승인 시간';
COMMENT ON COLUMN payments.cancelled_at IS '결제 취소 시간';
COMMENT ON COLUMN payments.cancel_reason IS '취소 사유';
COMMENT ON COLUMN payments.created_at IS '생성 시간';
COMMENT ON COLUMN payments.updated_at IS '수정 시간';

COMMENT ON COLUMN consultation_room.is_paid IS '결제 완료 여부 (0: 미결제, 1: 결제완료)';
